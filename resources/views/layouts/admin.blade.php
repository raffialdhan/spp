...
<head>
    ...
    <title>Sistem SPP - Administrator</title>
    ...
    <aside class="bg-[#362773] text-white flex flex-col justify-between transition-all duration-300 z-20 shrink-0 relative"
        ...
        <div class="w-72 relative z-10">
            ...
            <div class="h-24 flex items-center justify-between px-6 border-b border-white/10">
                <div class="flex items-center gap-4">
                    ...
                    <div x-show="sidebarOpen" x-transition.opacity class="whitespace-nowrap">
                        ...
                        <p class="text-[11px] font-medium text-indigo-200 mt-0.5 uppercase tracking-wider">Manajemen Sekolah</p>
                    </div>
                </div>
            ...
            <nav class="p-4 space-y-2 mt-4">
                ...
                <a href="/admin/users" class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->is('admin/users*') ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold shadow-lg shadow-purple-500/30 transform hover:scale-[1.02] hover:shadow-purple-500/40 relative overflow-hidden group' : 'text-indigo-200 hover:text-white hover:bg-white/10 font-medium' }}">
                    @if(request()->is('admin/users*'))
                        <div class="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-500 ease-out skew-x-12"></div>
                    @endif
                    <i class="fa-solid fa-users-gear text-lg w-6 text-center relative z-10"></i>
                    <span x-show="sidebarOpen" x-transition.opacity class="relative z-10">Manajemen Pengguna</span>
                </a>

                <a href="/admin/students" class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->is('admin/students*') ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold shadow-lg shadow-purple-500/30 transform hover:scale-[1.02] hover:shadow-purple-500/40 relative overflow-hidden group' : 'text-indigo-200 hover:text-white hover:bg-white/10 font-medium' }}">
                    @if(request()->is('admin/students*'))
                        <div class="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-500 ease-out skew-x-12"></div>
                    @endif
                    <i class="fa-solid fa-user-graduate text-lg w-6 text-center relative z-10"></i>
                    <span x-show="sidebarOpen" x-transition.opacity class="relative z-10">Data Master Siswa</span>
                </a>

                <a href="/admin/fees" class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->is('admin/fees*') ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold shadow-lg shadow-purple-500/30 transform hover:scale-[1.02] hover:shadow-purple-500/40 relative overflow-hidden group' : 'text-indigo-200 hover:text-white hover:bg-white/10 font-medium' }}">
                    @if(request()->is('admin/fees*'))
                        <div class="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-500 ease-out skew-x-12"></div>
                    @endif
                    <i class="fa-solid fa-money-check-dollar text-lg w-6 text-center relative z-10"></i>
                    <span x-show="sidebarOpen" x-transition.opacity class="relative z-10">Konfigurasi SPP</span>
                </a>
            </nav>
        </div>
            ...
            <div class="flex items-center gap-6">
                ...
                <div x-data="{ notifOpen: false }" class="relative">
                    ...
                    <div x-show="notifOpen" @click.away="notifOpen = false" x-cloak
                        ...
                        <div class="px-5 py-3 border-b border-slate-100 flex justify-between items-center">
                            <h3 class="font-bold text-slate-800">Notifikasi Sistem</h3>
                            <a href="#" class="text-xs text-indigo-600 font-bold hover:underline">Tandai dibaca</a>
                        </div>
                        <div class="max-h-80 overflow-y-auto">
                            <a href="#" class="block px-5 py-4 hover:bg-slate-50 transition-colors border-b border-slate-50 relative">
                                <div class="absolute left-0 top-0 bottom-0 w-1 bg-indigo-500"></div>
                                <p class="text-sm font-bold text-slate-800 mb-0.5">Laporan Bulanan Tersedia</p>
                                <p class="text-xs text-slate-500">Laporan pemasukan bulan April siap diunduh.</p>
                                <p class="text-[10px] text-slate-400 mt-2 font-medium">1 jam yang lalu</p>
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div x-data="{ dropdownOpen: false }" class="relative">
                    <button @click="dropdownOpen = !dropdownOpen" class="flex items-center gap-3 hover:bg-slate-100 p-1.5 pr-4 rounded-full transition-all border border-transparent hover:border-slate-200">
                        <div class="w-10 h-10 rounded-full bg-indigo-900 text-white flex items-center justify-center font-bold shadow-md text-sm">
                            {{ strtoupper(substr(Auth::user()->name ?? 'U', 0, 1)) }}
                        </div>
                        <div class="text-left hidden md:block">
                            <p class="text-sm font-bold text-slate-800 leading-tight">{{ Auth::user()->name ?? 'Pengguna' }}</p>
                            <p class="text-xs font-medium text-slate-500 mt-0.5">{{ ucfirst(Auth::user()->role ?? 'admin') }}</p>
                        </div>
