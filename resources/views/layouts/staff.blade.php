...
<head>
    ...
    <title>Sistem SPP - Beranda Petugas</title>
    ...
    <aside class="bg-[#362773] text-white flex flex-col justify-between transition-all duration-300 z-20 shrink-0 relative"
        ...
        <div class="w-72 relative z-10">
            ...
            <nav class="p-4 space-y-2 mt-4">
                ...
                <a href="/staff/verification" class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->is('staff/verification') ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold shadow-lg shadow-purple-500/30 transform hover:scale-[1.02] hover:shadow-purple-500/40 relative overflow-hidden group' : 'text-indigo-200 hover:text-white hover:bg-white/10 font-medium' }}">
                    @if(request()->is('staff/verification'))
                        <div class="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-500 ease-out skew-x-12"></div>
                    @endif
                    <i class="fa-solid fa-check-circle text-lg w-6 text-center relative z-10"></i>
                    <span x-show="sidebarOpen" x-transition.opacity class="relative z-10">Verifikasi Pembayaran</span>
                </a>
                <a href="/staff/payment" class="flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all {{ request()->is('staff/payment') ? 'bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold shadow-lg shadow-purple-500/30 transform hover:scale-[1.02] hover:shadow-purple-500/40 relative overflow-hidden group' : 'text-indigo-200 hover:text-white hover:bg-white/10 font-medium' }}">
                    @if(request()->is('staff/payment'))
                        <div class="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-500 ease-out skew-x-12"></div>
                    @endif
                    <i class="fa-regular fa-credit-card text-lg w-6 text-center relative z-10"></i>
                    <span x-show="sidebarOpen" x-transition.opacity class="relative z-10">Pembayaran Langsung</span>
                </a>
            </nav>
        </div>
                ...
                <div x-data="{ notifOpen: false }" class="relative">
                    ...
                    <div x-show="notifOpen" @click.away="notifOpen = false" x-cloak
                        ...
                        <div class="max-h-80 overflow-y-auto">
                            <a href="/staff/verification" class="block px-5 py-4 hover:bg-slate-50 transition-colors border-b border-slate-50 relative">
                                <div class="absolute left-0 top-0 bottom-0 w-1 bg-amber-500"></div>
                                <p class="text-sm font-bold text-slate-800 mb-0.5">Menunggu Verifikasi</p>
                                <p class="text-xs text-slate-500">Ahmad Fauzi mengirimkan bukti pembayaran baru.</p>
                                <p class="text-[10px] text-slate-400 mt-2 font-medium">10 menit yang lalu</p>
                            </a>
                        </div>
                    </div>
                </div>
